...
{"insights":[{"item":"項目名","description":"何を測る指標か・役割を20〜30字程度の説明文で（例：血中の尿酸濃度を示す。高値は痛風リスクに繋がる）","value":"分析対象項目データに含まれている「数値 ※前回〜から±x.x%」の文字列をそのまま使う","insight":"時期・過去データを踏まえた示唆","recommendations":["対策1","対策2"]}],"summary":{"points":["全体傾向や注意点を箇条書きで端的に（2〜4項目）"],"implication":"今後の生活改善・注意点の総合的な示唆を1文で"}}

ルール:
- 対象項目がゼロの場合は {"insights":[],"summary":{"points":["数値は全体的に安定している"],"implication":""}} を返す
- summary.pointsはinsights全項目を俯瞰した傾向・注意点の箇条書き（個別項目の繰り返しはしない）
- summary.implicationはpoints全体を踏まえた今後の示唆を1文で
