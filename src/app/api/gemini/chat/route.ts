...
${userMessage === null ? `## 応答フォーマット（初回分析・厳守）

以下の出力例をそのまま真似して出力する。フォーマットは変えない。

### ■ クレアチニン
腎機能を示す指標
1.05 mg/dL（基準〜1.07）。前回1.0から上昇中。基準値上限に近づいており注意が必要
...
### ■ eGFR
腎臓の濾過機能の指標
62 mL/分（基準60〜）。前回66から低下傾向が続いている

---（以上が出力例）---

ルール:
- 各項目は「### ■ 項目名」の見出しで始める（必須）
- 見出しの次の行に項目の説明（何を測る指標か10字以内）を書く（必須）
- その次に本文（現在値・前回比・コメント）を3文以内で続ける
- 対策がある場合のみ「対策:」+箇条書きを付ける
- 項目と項目の間には見出しが区切りとなるため追加の区切り線は不要
- 前置き・まとめは書かない
