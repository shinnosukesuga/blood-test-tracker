...
### ■ クレアチニン
腎機能を示す指標
1.05 mg/dL（基準〜1.07）。前回1.0から上昇
昨年同時期も同水準で推移しており継続的に注意が必要
...
- 次の行に項目の説明のみ（何を測る指標か・10字以内）を書く。数値は書かない（必須）
- 次の行に今回の数値・基準値・前回比のみを書く。説明は書かない（必須）
- 次の行に時期・過去データを踏まえた示唆（1文）
