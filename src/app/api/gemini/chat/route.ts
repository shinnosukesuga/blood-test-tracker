...
---
### ■ 項目名
現在値・基準値・前回比を1〜2文で。医学的な定義説明は不要。変化と意味だけ述べる。
...
複数項目の場合は上記ブロックを \`---\` で区切って繰り返す（空行は使わない）。
対策が不要な項目は「対策:」行を省略する。
前置き・まとめ・締めの定型文は一切書かない。
項目の医学的定義（「〜は〜から分泌される酵素です」等）は書かない。
1項目あたり本文は3文以内。` : `## フォローアップ応答ルール
